...
export class DataService {
  ...
  public getSubject(subjectUrl: string): Observable<Subject> {
    return this.httpClient
      .get<Subject>(`${this.SUBJECTS_ENDPOINT}/${subjectUrl}/populated`)
      .pipe(
        map((response) => {
  ...
  public getPost(postUrl: string): Observable<Post> {
    return this.httpClient.get<Post>(`${this.POSTS_ENDPOINT}/${postUrl}`).pipe(
      map((response) => {
  ...
  public getMultiplePosts(postIdsString: string): Observable<Post[]> {
    return this.httpClient
      .get<Post[]>(`${this.POSTS_ENDPOINT}/multiple/${postIdsString}`)
      .pipe(
        map((response) => {
  ...
  public getAllLessons(): Observable<string[]> {
    return this.httpClient
      .get<string[]>(`${this.POSTS_ENDPOINT}/all-lessons`)
      .pipe(
        map((response) => {
  ...
  public getImage(imageId: string): Observable<ImageData> {
    return this.httpClient
      .get<ImageData>(`${this.IMAGES_ENDPOINT}/${imageId}`)
      .pipe(
        map((response) => {
  ...
  public getMultipleImages(
    ...
    return this.httpClient
      .get<ImageFile[]>(`${this.IMAGES_ENDPOINT}/all`, { headers, params })
      .pipe(
        map((response) => {
  ...
  public uploadImage(file: any): Observable<any> {
    ...
    return this.httpClient
      .post<any>(`${this.IMAGES_ENDPOINT}/upload`, formData)
      .pipe(
        map((response) => {
  ...
  public deleteImageById(id: string): Observable<any> {
    return this.httpClient
      .delete<any>(`${this.IMAGES_ENDPOINT}/${id}/delete`)
      .pipe(
        map((response) => {
  ...
  public getAllSchoolWeeks(): Observable<SchoolWeek[]> {
    return this.httpClient
      .get<SchoolWeek[]>(`${this.SCHOOL_WEEK_ENDPOINT}/all`)
      .pipe(
        map((response) => {
  ...
  public getAllNews(page = 0, itemsPerPage = 10): Observable<SchoolNews[]> {
    ...
    return this.httpClient
      .get<SchoolNews[]>(`${this.NEWS_ENDPOINT}/all`, { headers, params })
      .pipe(
        map((response) => {
